...
![Player Node States](client-states.png){width=75%}
...
We tested this by running multiple clients and observing if the updates being sent from one client was being applied by others or not. This was confirmed when a player moving and shooting on 1 screen was shown to do the exact thing almost instantly on the other screens.
...
We tested this by looking at the logs generated by GoVector and displayed in Shiviz and making sure the protocol was behaving correctly. We also tested this by making sure that each player had valid stats from the game before in the KV store which was done by reading all the files that nodes wrote to save their set of keys. Additionally, we simulated disconnected clients and in the event of no connected replica of a key-value pair, the server returned a default value of 0 as expected.

### Disconnections

We tested disconnections by either killing or suspending the processes on player nodes. The game survived these disconnections. If our game had n players and n-1 other players disconnected, our game still survived with the last player and bot remaining in the game. So, our game was indeed pretty fault tolerant. That being said, if the server went down, our game crashed the next time any of the clients made any kind of request to the server. But, a failing server is a violation of our assumption so we don't really care about this.
